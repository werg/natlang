...
The compiler enumerates value bindings visible at the `nl` expression: parent parameters, persistent eval locals, earlier declarations in the current eval, and bindings in surrounding blocks. Normal lexical shadowing applies. A later declaration, an out-of-scope block local, and a type-only alias cannot be captured. When created during a named `foo.nl` invocation or in a callable TypeScript file under `foo/`, the anonymous lambda sees the same callable items under `foo/` as its parent. When created in ordinary application code, it sees the callable items of the nearest ancestor `natlang.d/` folder, or none if there is no such folder. Callable TypeScript files there may import sibling `.ts` and `.nl` functions and call anonymous natlang lambdas in their own code. The explicit positional parameters of a call shadow captured names, just as function parameters shadow outer variables.
...
The target semantics are normal closure semantics: a child can read captured bindings; writes to a captured `let` or a property of a captured object become visible to the parent after the child action succeeds; rebinding a captured `const` is rejected. The runtime must expose typed getter/setter cells or live object handles to the child instead of constructing an independent input snapshot. Child edits respect the atomic action boundary: a captured `let` is read at the start of each eval action and its rebinding is written back only if the action succeeds. Write-back is version-checked, so if a sibling changed the cell in the meantime the action fails with a conflict and the child retries its eval. No lock is held across a child call, so parallel children that read the same capture never wait. Property writes on live objects are immediate and traced, not rolled back. This is runtime work following the compiler task, not a reason to freeze captures in the language design.
...
5. **Iteration step.** For `nl`...`.iterateOn(initial, ...args)` and `iterateOn(nl`...`, initial, ...args)`, the state parameter has the initial value's type and the step returns it. An unannotated `nl` function's `iterateOn` is generic in the state, so a stopping check `until(nl`...`)` gets `(state: S) => boolean` from context.
6. **Uses within the same file (typed holes).** When 1–5 leave the result type open, the compiler checks a second program where each such tag has a unique placeholder result type, as in Rust or ML local inference. The placeholder flows through ordinary TypeScript (locals, arrays, `Promise.all`, element access, `for...of`, saved functions), and every use of a placeholder-typed value adds a requirement: the type TypeScript expects there (assignments, arguments, returns, `push`, object fields), matched structurally through arrays, promises, tuples, and objects; or what the syntax implies where TypeScript expects nothing (a condition, `!`, or a `filter`/`some`/`every`/`find` callback result needs `boolean`; arithmetic and numeric comparison need `number`; a string or number method names its type). Those are strong requirements. Weak ones apply only when there is no strong one: text use (template interpolation, concatenation with a string, `console` output, an eval's displayed final expression) gives `string`, and comparison with a value gives that value's type (a literal's base type). The most specific requirement that satisfies all the others wins (`"low" | "high"` over `string`). Disagreeing requirements are reported with the statements that impose them; no requirement at all is reported with a proposed annotation (for a result read only by field, `nl<{ severity: …; reason: … }>`). Implementation: `ts-host/src/compiler/holes.ts`.
