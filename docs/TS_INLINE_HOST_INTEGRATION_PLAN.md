...
1. **Freeze the target contract.** Add executable examples for immediate calls, typed callbacks, closure reads/writes, nested calls, a directory reducer, scoped `foo.nl` children, sibling `.ts` to `.nl` imports and inline calls, rejected out-of-scope calls, recursion errors through `.ts`, and Node/browser parity. Update the spec with the new source and invocation contract. Gate: every example has a pre-run target type and one expected trace shape.
...
- **Callable scope:** a named `.nl` lambda and its inline descendants see the same callable items under its companion folder. Callable `.ts` items can import `.ts`/`.nl` siblings and create inline natlang lambdas with that same context. The compiler's import graph cannot broaden that namespace, and runtime checks reject forged or aliased out-of-scope handles.
