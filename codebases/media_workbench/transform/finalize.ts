...
returns: MediaResult
---*/
const p = args.plan, s = args.source, r = args.receipt, i = args.inspection, a = args.assessment;
...
const technical_ok = r.status === 'ok' && i.status === 'ok' && r.output === args.request.output &&
...
return { status, output: args.request.output, technical_ok, semantic_ok,
