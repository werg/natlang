...
On its first semantic turn, an unconfigured CLI lazily starts and owns a local
`llama-server`, using the verified static default generated by the most recent
model publication. The publication command updates browser and CLI defaults
together. Setup validates an existing llama.cpp runtime or asks before
installing natlang's pinned, verified user local build.
