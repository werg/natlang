...
browser, packages, CLI, or terminal applications. Then use the checkout-local
commands. Applications are addressed by source path; there is no precompiled
development app list:

```bash
scripts/natlang doctor --json
...
The wrappers rebuild changed Node TypeScript automatically. On its first
semantic turn, an unconfigured CLI lazily starts and owns a local
