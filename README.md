...
Use `scripts/setup_dev.sh --node-only` when working only on the TypeScript host,
browser, packages, CLI, or terminal applications. Setup installs the development
`natlang` command in `~/.local/bin`; it points at this checkout and rebuilds
changed TypeScript automatically. Applications are addressed by source path;
...
the log investigator includes `/demo`, and the semantic terminal exposes its
exact host operations through `/recipes`.

When a reducer-backed application starts, the CLI immediately prepares and owns
a local `llama-server`, using the verified static default generated by the most
recent model publication. Model startup overlaps application module and state
initialization, and the interactive application opens after the model is ready.
Direct source programs also begin model preparation immediately, concurrently
with source loading. The publication command updates browser and CLI defaults
together.
See [Development setup](DEV_SETUP.md) for model lifecycle, overrides, every
application, browser builds, and tests.
...
| File | Contents |
|------|----------|
| [Development setup](DEV_SETUP.md) | One-command setup, automatic rebuilds, model configuration, application launchers, and tests |
...
.venv/bin/python tools/check_conformance.py  # suite and specification type expressions
```
...
The published browser catalog is `models/browser-catalog.json`. It is written
only after GGUF conversion and verification succeed. The same publication
regenerates `ts-host/src/model-default.ts`, so browser and CLI builds select the
same verified checkpoint without resolving a mutable pointer at runtime. The
playground and browser applications read the catalog; the managed CLI uses the
generated static record. The catalog and generated weights live in the local,
ignored `models/` directory; copy them with the matching template to a browser
deployment. Set `NATLANG_MODEL_DOWNLOAD_URL` when running either training
wrapper, or pass `--download-url` to the publication command, when installed
CLI packages must be able to fetch the artifact. Direct calls to
`scripts/train_lora.py` still produce a merged checkpoint; run
`scripts/publish_browser_model.mjs --run runs/NAME --name NAME` afterward to
