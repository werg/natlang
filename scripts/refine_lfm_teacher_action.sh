...
DATA="${1:-data/lfm25-v8-weakness-live-action-targeted-v1.train.jsonl}"
START_ADAPTER="${2:-runs/lfm25-teacher-v8-phase2-from0273/checkpoint/weights}"
OUT="${3:-runs/lfm25-teacher-v8-live-action}"
LABEL="${4:-lfm25-8b-live-action}"
...
docker run --rm --gpus all -v "$ROOT:/work" -w /work -e HF_HOME=/work/models/hf \
  ...
  --token-cache data/tokens-lfm25-v8-live-action-v1.sqlite
