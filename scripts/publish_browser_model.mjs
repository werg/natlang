...
export async function promoteBrowserModel({ root = repository, run, artifact, name, quant = 'Q4_K_M',
  template, contextTokens = 8192, moveArtifact = false }) {
  root = resolve(root);
  ...
  const entry = { id: stem.toLowerCase() + '-' + quant.toLowerCase(),
    ...
    contextTokens, url: `/models/${modelFile}` };
...
async function main() {
  ...
  if (!run || !name || !slug(name) || !['Q4_K_M', 'Q5_K_M', 'Q6_K', 'Q8_0'].includes(quant))
    throw new Error('usage: publish_browser_model.mjs --run runs/NAME --name SLUG [--quant Q4_K_M] [--template PATH] [--artifact models/FILE.gguf]');
