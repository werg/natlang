...
def recipe(repo, model="LiquidAI/LFM2.5-350M", revision=None, image=None, python="python", sources=None,
           ...
           teacher_model="Ternary-Bonsai-2-27B", teacher_server="http://127.0.0.1:8081", token_file=None, train_args=(), init_adapter=None, min_free_vram_mib=2048, inventories_override=None):
    ...
    add("assemble", ["node", f"{p}/ts-host/scripts/code-corpus/assemble-shards.mjs", f"{r}/bundle", f"{r}/sources", *inventories],
        [f"{r}/sources/manifest.json", f"{p}/ts-host/scripts/code-corpus/assemble.mjs", *inventories], [f"{r}/bundle/complete.jsonl", f"{r}/bundle/train.jsonl", f"{r}/bundle/test.jsonl"])
    ...
    add("observe-source", ["node", f"{p}/ts-host/scripts/code-corpus/source-cases.mjs", "--output", f"{r}/source-observations.jsonl", "--execute", "--limit", "5000",
        ...
        [f"{p}/ts-host/scripts/code-corpus/source-cases.mjs", *observation_inputs],
    ...
    def render(name, source):
        add(f"render-{name}", py([f"{p}/scripts/render_training_corpus.py", "--inputs", source, "--output", f"{r}/{name}.sft.jsonl", *model_args]),
            [f"{p}/scripts/render_training_corpus.py", source], [f"{r}/{name}.sft.jsonl", f"{r}/{name}.sft.jsonl.manifest.json"])
    def train(name, previous, lr):
        checkpoint = f"{r}/train-{name}/checkpoint/state.json"
        inputs = [f"{r}/{name}.sft.jsonl", f"{r}/{name}.sft.jsonl.manifest.json", f"{p}/scripts/train_lora.py", f"{p}/natlang/corpus.py"]
        args = [f"{p}/scripts/train_lora.py", f"{r}/{name}.sft.jsonl", f"{r}/train-{name}", *train_model_args,
...
def main():
    ...
    config = recipe(Path(__file__).resolve().parents[1], args.model, args.revision, args.image, args.python,
                    ...
                    inventories_override=args.inventory)
