#!/usr/bin/env bash
# Resume-safe hard-curriculum training, teacher polish, and readiness gate.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

START_ADAPTER="${1:-runs/lfm25-teacher-v8-phase2-from0273/checkpoint/weights}"
HARD_OUT="${2:-runs/lfm25-teacher-v8-hard-after-teacher}"
POLISH_OUT="${3:-runs/lfm25-teacher-v8-hard-teacher-polish}"
LABEL="${4:-lfm25-8b-hard-teacher-polish}"
...
docker run --rm --gpus all -v "$ROOT:/work" -w /work -e HF_HOME=/work/models/hf \
  ...
  data/lfm25-v8-hard-curriculum-v1.train.jsonl "$HARD_OUT" \
  "${COMMON[@]}" --init-adapter "$START_ADAPTER" --epochs 0.15 --lr 5e-5 \
  --holdout 400 --save-every 50 --snapshot-every 100 \
  --token-cache data/tokens-lfm25-v8-hard-curriculum-v1.sqlite

docker rm -f natlang-train-polish >/dev/null 2>&1 || true
docker run --rm --gpus all -v "$ROOT:/work" -w /work -e HF_HOME=/work/models/hf \
  --name natlang-train-polish natlang-train python scripts/train_lora.py \
  data/lfm25-teacher-reviewed-exact.train.jsonl "$POLISH_OUT" \
  "${COMMON[@]}" --init-adapter "$HARD_OUT/checkpoint/weights" --epochs 1 --lr 2e-5 \
  --holdout 100 --save-every 25 --snapshot-every 25 \
  --token-cache data/tokens-lfm25-v8-hard-teacher-polish.sqlite
