...
def _script(actions):
    ...
    for a in actions:
        ...
        outs.append(header + "\n")
        if header.split()[0] in ("set", "edit", "eval", "reopen", "close"):
            outs.append(body)
    return outs
...
def test_two_phase_decoding_runs_a_program():
    ...
    assert "<|tool_call_start|>" in dec.calls[0][0] and "instructions  Text" in dec.calls[0][0]
...
def test_close_quiesces_with_the_note():
