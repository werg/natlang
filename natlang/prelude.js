...
const groupBy = (xs, f) => { const k = _key(f), out = {}; for (const x of xs) (out[k(x)] ||= []).push(x); return out; };
const countBy = (xs, f) => { const k = f ? _key(f) : (x) => x, out = {}; for (const x of xs) out[k(x)] = (out[k(x)] || 0) + 1; return out; };
...
const indexBy = (xs, f) => { const k = _key(f), out = {}; for (const x of xs) out[k(x)] = x; return out; };
