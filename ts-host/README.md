...
The local pilot server sends COOP/COEP headers and supports byte ranges for the GGUF; the page uses up to four WASM threads when isolated. The adapter converts prior tool-call arguments from JSON strings into objects for the official LFM template. Initial headless CPU trials loaded v8 in about four seconds but timed out before a first tool response under the three-minute task limit, including a 128-token turn with four threads. The host was under heavy CPU load from other jobs, so these trials do not establish a clean speed measurement or model accuracy. Headless Chromium reported no WebGPU adapter; use a browser with a real adapter to measure GPU behavior.
