...
Use `--suite` for all three pilot tasks, or `--task=record` / `--task=nested-map` to run one structured case. Structured cases use a 4,096-token context by default in the CLI; `--context=8192` overrides it. `--probe=plain`, `--probe=tool`, and `--probe=natlang` isolate base generation, a simple tool, and the exact natlang request; `--probe-tokens=N` changes the probe limit. The model manifest pins the local v8 GGUF SHA-256 digest, while the page reports storage estimates and lets a user select a GGUF file.
