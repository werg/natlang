...
export const crispExamples = [
  crisp({ id: 'structured-calculation', name: 'Structured calculation', category: 'Starter',
    ...
    args: { a: 'Num', b: 'Num' }, returns: 'Summary',
    files: { 'math/types.ts': 'type Summary = { sum: Num, larger: Num };\n' },
    code: 'return { sum: args.a + args.b, larger: Math.max(args.a, args.b) };',
  ...
  crisp({ id: 'checked-greeting', name: 'Checked greeting', category: 'Starter',
    ...
    args: { person: 'Person' }, returns: 'Text',
    files: { 'people/types.ts': 'type Person = { name: Text };\n' },
    code: 'return "Hello, " + args.person.name + "!";',
  ...
  crisp({ id: 'invoice-totals', name: 'Invoice totals', category: 'Data',
    ...
    args: { lines: 'Line[]', tax_rate: 'Num' }, returns: 'Invoice',
    files: { 'invoice/types.ts': 'type Line = { label: Text, price: Num, quantity: Num };\ntype Invoice = { subtotal: Num, tax: Num, total: Num };\n' },
    code: `const subtotal = args.lines.reduce((n, line) => n + line.price * line.quantity, 0);
const tax = Math.round(subtotal * args.tax_rate * 100) / 100;
return { subtotal, tax, total: Math.round((subtotal + tax) * 100) / 100 };`,
  ...
  crisp({ id: 'tiered-discount', name: 'Tiered discount', category: 'Data',
    ...
    args: { amount: 'Num', member: 'Bool' }, returns: 'Quote',
    files: { 'pricing/types.ts': 'type Quote = { discount: Num, due: Num };\n' },
    code: `const rate = args.member ? (args.amount >= 100 ? 0.2 : 0.1) : 0;
const discount = Math.round(args.amount * rate * 100) / 100;
return { discount, due: Math.max(0, Math.round((args.amount - discount) * 100) / 100) };`,
  ...
  crisp({ id: 'word-histogram', name: 'Word histogram', category: 'Data',
    ...
    args: { text: 'Text' }, returns: 'Dict<Num>',
    code: `const counts = {};
for (const word of args.text.toLowerCase().match(/[a-z]+/g) ?? []) counts[word] = (counts[word] ?? 0) + 1;
return counts;`,
  ...
  crisp({ id: 'stable-deduplication', name: 'Stable deduplication', category: 'Data',
    ...
    args: { items: 'Text[]' }, returns: 'Text[]',
    code: 'return [...new Set(args.items)];',
  ...
  crisp({ id: 'shared-tags', name: 'Shared tags', category: 'Data',
    ...
    args: { left: 'Text[]', right: 'Text[]' }, returns: 'Text[]',
    code: 'const right = new Set(args.right); return [...new Set(args.left.filter(tag => right.has(tag)))].sort();',
  ...
  crisp({ id: 'csv-measurements', name: 'CSV measurements', category: 'Data',
    ...
    args: { csv: 'Text' }, returns: 'Measurement',
    files: { 'data/types.ts': 'type Measurement = { count: Num, sum: Num, mean: Num };\n' },
    code: String.raw`const rows = args.csv.trim().split(/\r?\n/).slice(1);
...
return { count: values.length, sum, mean: values.length ? sum / values.length : 0 };`,
  ...
  crisp({ id: 'rolling-average', name: 'Rolling average', category: 'Data',
    ...
    args: { values: 'Num[]', width: 'Num' }, returns: 'Num[]',
    code: `const out = [];
for (let i = 0; i + args.width <= args.values.length; i++) {
  out.push(args.values.slice(i, i + args.width).reduce((a, b) => a + b, 0) / args.width);
}
return out;`,
...
return stack.length === 0;`,
  ...
  crisp({ id: 'shortest-route', name: 'Shortest route', category: 'Algorithms',
    ...
    args: { graph: 'Dict<Text[]>', start: 'Text', goal: 'Text' }, returns: 'Text[]',
    code: `const queue = [[args.start]];
const seen = new Set([args.start]);
for (const path of queue) {
  const node = path[path.length - 1];
  if (node === args.goal) return path;
  for (const next of args.graph[node] ?? []) if (!seen.has(next)) {
...
return [];`,
  ...
  crisp({ id: 'dependency-order', name: 'Dependency order', category: 'Algorithms',
    ...
    args: { tasks: 'Task[]' }, returns: 'Text[]',
    files: { 'algorithms/types.ts': 'type Task = { id: Text, needs: Text[] };\n' },
    code: `const pending = new Map(args.tasks.map(task => [task.id, task.needs]));
const order = [];
while (pending.size) {
...
return order;`,
  ...
  crisp({ id: 'merge-intervals', name: 'Merge intervals', category: 'Algorithms',
    ...
    files: { 'algorithms/types.ts': 'type Range = { start: Num, end: Num };\n' },
    code: `const sorted = [...args.ranges].sort((a, b) => a.start - b.start);
const merged = [];
...
return merged;`,
  ...
  crisp({ id: 'event-reconciliation', name: 'Event reconciliation', category: 'Workflows',
    ...
    args: { sent: 'Text[]', acknowledged: 'Text[]' }, returns: 'Reconciliation',
    files: { 'workflow/types.ts': 'type Reconciliation = { confirmed: Text[], pending: Text[], unknown: Text[] };\n' },
    code: `const sent = new Set(args.sent), acknowledged = new Set(args.acknowledged);
return { confirmed: [...sent].filter(id => acknowledged.has(id)).sort(),
  pending: [...sent].filter(id => !acknowledged.has(id)).sort(),
  unknown: [...acknowledged].filter(id => !sent.has(id)).sort() };`,
  ...
  crisp({ id: 'ticket-reducer', name: 'Ticket state reducer', category: 'Workflows',
    ...
    args: { events: 'Text[]' }, returns: 'TicketState',
    files: { 'workflow/types.ts': 'type TicketState = { status: Text, escalated: Bool, transitions: Num };\n' },
...
return { status, escalated, transitions };`,
  ...
  crisp({ id: 'priority-queue', name: 'Priority queue', category: 'Workflows',
    ...
    args: { tasks: 'QueueTask[]' }, returns: 'Text[]',
    files: { 'workflow/types.ts': 'type QueueTask = { id: Text, priority: Num, age: Num, blocked: Bool };\n' },
    code: `return args.tasks.filter(task => !task.blocked)
  ...
  crisp({ id: 'safe-rollout', name: 'Safe rollout gate', category: 'Workflows',
    ...
    args: { tests_passed: 'Bool', error_rate: 'Num', latency_ms: 'Num' }, returns: 'Gate',
    files: { 'workflow/types.ts': 'type Gate = { proceed: Bool, reasons: Text[] };\n' },
    code: `const reasons = [];
if (!args.tests_passed) reasons.push('tests failed');
if (args.error_rate > 0.02) reasons.push('error rate high');
if (args.latency_ms > 500) reasons.push('latency high');
return { proceed: reasons.length === 0, reasons };`,
  ...
  crisp({ id: 'record-diff', name: 'Record change summary', category: 'Data',
    ...
    args: { before: 'Dict<Text>', after: 'Dict<Text>' }, returns: 'Changes',
    files: { 'data/types.ts': 'type Changes = { added: Text[], removed: Text[], changed: Text[] };\n' },
    code: `const left = args.before, right = args.after;
return { added: Object.keys(right).filter(key => !(key in left)).sort(),
  ...
  crisp({ id: 'markdown-outline', name: 'Markdown outline', category: 'Text',
    level: 'Intermediate', description: 'Extract headings with their levels.',
    concepts: ['text parsing', 'records'], root: 'text/outline.ts',
    args: { markdown: 'Text' }, returns: 'Heading[]',
    files: { 'text/types.ts': 'type Heading = { level: Num, title: Text };\n' },
    code: String.raw`return args.markdown.split(/\r?\n/).flatMap(line => {
  ...
  crisp({ id: 'redact-identifiers', name: 'Redact identifiers', category: 'Text',
    ...
    args: { text: 'Text' }, returns: 'Text',
    code: String.raw`return args.text.replace(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, '[email]')
  ...
  crisp({ id: 'form-validation', name: 'Form validation', category: 'Workflows',
    ...
    args: { name: 'Text', email: 'Text', age: 'Num' }, returns: 'Dict<Text>',
    code: String.raw`const errors = {};
if (!args.name.trim()) errors.name = 'required';
if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(args.email)) errors.email = 'invalid email';
if (args.age < 18) errors.age = 'must be 18 or older';
return errors;`,
