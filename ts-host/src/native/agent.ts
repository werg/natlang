...
export class NativeToolAgent {
  ...
  private toolsScope(session: NativeSession): any[] {
    const tools = [
      ...
      tool('return_value', 'The normal way to finish: stage one existing variable as this function\'s typed result. After eval creates `answer`, call return_value(variable="answer"); do not transport or wrap it with write_value. End the turn naturally after all instruction lines are closed.',
        { variable: { type: 'string', pattern: '^[A-Za-z_$][A-Za-z0-9_$]*$' } }, ['variable']),
      tool('write_value', 'Transport an already supplied literal into a NEW top-level scope variable. This never stages the function result and cannot overwrite a parameter, helper, or existing local. For normal program work, including literal decisions, prefer eval declarations. as_type is needed only when inference is ambiguous.',
