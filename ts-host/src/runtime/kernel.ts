...
export async function invokeDefinition(frame: Frame, definition: CallableDefinition, positional: unknown[],
  ...
  const agent = model ? new NativeToolAgent(model.driver, { systemPrompt: () => task.systemPrompt(environment),
    ...
    review: model.review, validationFeedback: model.validationFeedback }) : undefined;
