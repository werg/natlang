...
export const ide = define({ id: 'ide', project: 'P06', title: 'Atelier', subtitle: 'A home for living programs', category: 'Create', icon: '⌘', color: '#b6a2ee',
    ...
    stateType: `export type File = { id: string; source: string }; export type Scenario = { id: string; input: string; expected: string; actual: string; status: string }; export type State = { ${COMMON} files: File[]; selected: string; result: string; trace_id: string; trace_count: number; trace_cursor: number; trace_frame: string; scenarios: Scenario[] };`,
...
export const notebook = define({ references:s=>s.cells.filter(c=>c.result_id).map(c=>({store:'native_values',id:c.result_id})), id: 'notebook', project: 'P10', title: 'Fieldnotes', subtitle: 'Think in cells, follow the evidence', category: 'Create', icon: '▤', color: '#e9bd79', panelIds: ['cells', 'lineage'],
    stateType: `export type Cell = { id: string; engine: string; source: string; needs: string[]; result: string; result_id: string; status: string }; export type State = { ${COMMON} cells: Cell[] };`,
    initial: () => initial({ cells: [{ id: 'numbers', engine: 'typescript', source: 'return [2, 3, 5, 7, 11];', needs: [], result: '', result_id: '', status: 'ready' }, { id: 'total', engine: 'typescript', source: 'return deps.numbers.reduce((sum, n) => sum + n, 0);', needs: ['numbers'], result: '', result_id: '', status: 'ready' }] }),
    ...
    async apply(s, d, host) {
        if (d.action === 'add') {
            ...
            check(['typescript', 'sqlite', 'natlang'].includes(d.secondary), 'Unknown engine');
    ...
    panels: s => [p('cells', 'Your notebook', 'cells', { cells: s.cells }), form('lineage', 'Add a cell', [f('target', 'Cell identifier', 'analysis'), f('secondary', 'Engine', 'typescript', 'select', ['typescript', 'natlang', 'sqlite']), f('ids', 'Dependencies (comma separated)', 'numbers'), f('text', 'Source', 'return deps.numbers.map(n => n * n);', 'code')], [a('Add cell', 'add')], { rows: s.cells.map(c => ({ cell: c.id, needs: c.needs.join(' → ') || 'source', status: c.status })), columns: ['cell', 'needs', 'status'] })],
    smoke: { action: 'add', target: 'extra', secondary: 'typescript', text: 'return 42;', ids: [] },
});
export const wiki = define({ id: 'wiki', project: 'P11', title: 'Commonplace', subtitle: 'Pages that think with you', category: 'Create', icon: '◈', color: '#8ac9b7', panelIds: ['pages', 'editor', 'read'],
    stateType: `export type Page = { id: string; title: string; body: string; cell: string; output: string }; export type State = { ${COMMON} pages: Page[]; selected: string; incoming: string; conflicts: string[] };`,
...
export const merge = define({ id: 'merge', project: 'P02', title: 'Confluence', subtitle: 'Explore what it means to agree', category: 'Explore', icon: '⋈', color: '#d0a5dd', panelIds: ['replicas', 'proposal', 'history'],
    stateType: `export type Entry = { id: string; replica: string; text: string }; export type State = { ${COMMON} family: string; base: string; left: string; right: string; merged: string; issues: string[]; history: Entry[] };`,
...
export const evidence = define({ id: 'evidence', project: 'P14', title: 'Atlas', subtitle: 'Ideas with a paper trail', category: 'Explore', icon: '◎', color: '#8ebbdc', panelIds: ['library', 'search', 'answer'],
    stateType: `export type Passage = { id: string; title: string; text: string }; export type Claim = { id: string; text: string; passage: string; quote: string }; export type State = { ${COMMON} passages: Passage[]; query: string; hits: string[]; claims: Claim[] };`,
...
export const publisher = define({ id: 'publisher', project: 'P18', title: 'Folio', subtitle: 'From scattered thoughts to a finished piece', category: 'Create', icon: '¶', color: '#e2aa96', panelIds: ['outline', 'compose', 'preview'],
    stateType: `export type Section = { id: string; heading: string; body: string }; export type State = { ${COMMON} title: string; sections: Section[]; published: boolean };`,
...
export const types = define({ id: 'types', project: 'P05', title: 'Type garden', subtitle: 'Make implicit contracts visible', category: 'Develop', icon: 'λ', color: '#a7c6a0', panelIds: ['source', 'contracts', 'diagnostics'],
    stateType: `export type Contract = { id: string; signature: string; reason: string }; export type Diagnostic = { id: string; location: string; detail: string }; export type State = { ${COMMON} source: string; contracts: Contract[]; diagnostics: Diagnostic[] };`,
