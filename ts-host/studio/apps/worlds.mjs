...
export const economy = define({ id: 'economy', project: 'P08', title: 'Market day', subtitle: 'Every exchange tells a story', category: 'Play', icon: '◉', color: '#e4bc72', panelIds: ['market', 'trade', 'ledger'],
    stateType: `export type Merchant = { id: string; name: string; cash: number; apples: number; price: number }; export type Trade = { id: string; buyer: string; seller: string; quantity: number; total: number }; export type State = { ${COMMON} merchants: Merchant[]; trades: Trade[]; tick: number };`,
...
export const combat = define({ id: 'combat', project: 'P08', title: 'Sparring grounds', subtitle: 'Read the moment. Choose your move.', category: 'Play', icon: '⚔', color: '#db9d9e', panelIds: ['arena', 'moves', 'rounds'],
    stateType: `export type Fighter = { id: string; hp: number; energy: number; stance: string }; export type Round = { id: string; player: string; rival: string; detail: string }; export type State = { ${COMMON} fighters: Fighter[]; rounds: Round[]; seed: number };`,
...
export const npc = define({ id: 'npc', project: 'P08', title: 'The lantern inn', subtitle: 'A conversation with a memory', category: 'Play', icon: '☷', color: '#c9b18e', panelIds: ['scene', 'conversation', 'memory'],
    stateType: `export type Memory = { id: string; text: string }; export type Line = { id: string; speaker: string; text: string; evidence: string[] }; export type State = { ${COMMON} memories: Memory[]; lines: Line[]; trust: number };`,
...
export const spells = define({ id: 'spells', project: 'P09', title: 'Spellweaver', subtitle: 'Words become a little bit of magic', category: 'Play', icon: '✧', color: '#b8a5ef', panelIds: ['arena', 'spell', 'grimoire'],
    stateType: `export type Spell = { id: string; words: string; element: string; power: number; cost: number }; export type State = { ${COMMON} mana: number; targetHp: number; ward: string; spells: Spell[]; last: string };`,
...
export const experiments = define({ id: 'experiments', project: 'P16', title: 'Possibility lab', subtitle: 'Same world. Different choices.', category: 'Explore', icon: '⎇', color: '#92c9ce', panelIds: ['design', 'chart', 'trials'],
    stateType: `export type Trial = { id: string; policy: string; seed: number; reward: number; steps: number }; export type State = { ${COMMON} trials: Trial[]; hypothesis: string };`,
