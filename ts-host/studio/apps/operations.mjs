...
export const scheduling = define({ id: 'scheduling', project: 'P19', title: 'Daylight', subtitle: 'Make room for what matters', category: 'Organize', icon: '◷', color: '#e1c185', panelIds: ['day', 'tasks', 'plan'],
    stateType: `export type Task = { id: Text; title: Text; duration: Num; start: Text; done: Bool }; export type State = { ${COMMON} tasks: Task[]; date: Text };`,
...
export const logs = define({ id: 'logs', project: 'P12', title: 'Signal room', subtitle: 'Find the story in the noise', category: 'Observe', icon: '⌁', color: '#90bdb4', panelIds: ['stream', 'incidents', 'escalation'],
    stateType: `export type Log = { id: Text; time: Text; level: Text; message: Text }; export type Incident = { id: Text; title: Text; evidence: Text[]; severity: Text; status: Text }; export type State = { ${COMMON} logs: Log[]; incidents: Incident[]; receipts: Text[] };`,
...
export const data = define({ id: 'data', project: 'P13', title: 'Data kitchen', subtitle: 'Bring messy records into alignment', category: 'Explore', icon: '▦', color: '#a9c79c', panelIds: ['input', 'mapping', 'output'],
    stateType: `export type Mapping = { id: Text; source: Text; target: Text }; export type State = { ${COMMON} input: Text; mappings: Mapping[]; output: Text; issues: Text[] };`,
...
export const workflows = define({ id: 'workflows', project: 'P15', title: 'Relay', subtitle: 'Make every step accountable', category: 'Organize', icon: '⇢', color: '#b0b6da', panelIds: ['flow', 'actions', 'receipts'],
    stateType: `export type WorkflowStep = { id: Text; title: Text; needs: Text[]; status: Text; key: Text }; export type Receipt = { id: Text; step: Text; status: Text }; export type State = { ${COMMON} steps: WorkflowStep[]; receipts: Receipt[] };`,
...
export const tests = define({ id: 'tests', project: 'P17', title: 'Counterexample', subtitle: 'The most useful answer might be a failure', category: 'Develop', icon: '◇', color: '#d4acb7', panelIds: ['contract', 'cases', 'results'],
    stateType: `export type Case = { id: Text; input: Text; expected: Text; actual: Text; status: Text }; export type State = { ${COMMON} source: Text; contract: Text; cases: Case[] };`,
    initial: () => initial({ source: '/*---\nengine: typescript-host\nargs:\n  values: Num[]\nreturns: Num\n---*/\nreturn args.values.reduce((a,b) => a+b, 0);', contract: 'Sum all numbers. An empty list sums to zero.', cases: [{ id: 'empty', input: '{"values":[]}', expected: '0', actual: '', status: 'pending' }, { id: 'positive', input: '{"values":[2,3]}', expected: '5', actual: '', status: 'pending' }] }),
    ...
    instructions: 'Build a behavior collection and seek counterexamples. save stores source text and contract secondary, invalidating results. add supplies case ID target, JSON input text, JSON expected secondary. run_case evaluates the target case using actual child execution. Natlang should propose boundary cases from the contract, but exact host execution determines results. Remove redundant examples only with a justified request; do not label generated cases as reviewed training data.',
