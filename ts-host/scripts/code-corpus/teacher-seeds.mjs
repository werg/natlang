...
export async function teacherSeeds(input, output, count = 1000, seed = 42) {
  ...
  const identity = {version:'natlang.teacher_seeds/1', input_sha256:digest(captured), count, seed,
...
if(process.argv[1] && import.meta.url===pathToFileURL(resolve(process.argv[1])).href) {
  const [input, output, count='1000', seed='42'] = process.argv.slice(2);
  if(!input||!output) throw new Error('Usage: teacher-seeds.mjs PROJECTED_PROGRAMS OUTPUT [COUNT] [SEED]');
  if(!Number.isSafeInteger(Number(count))||Number(count)<0) throw new Error('COUNT must be nonnegative');
  console.log(JSON.stringify(await teacherSeeds(input, output, Number(count), Number(seed))));
}
