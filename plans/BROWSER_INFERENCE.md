...
A Vulkan-enabled headless Chromium pilot exposed a WebGPU adapter, requested all layers, and completed the leaf task in 77 seconds. Its adapter lacked shader-f16, Wllama did not report the actual offload count, and GPU memory observation did not show a model-sized allocation. The updated capability check now selects CPU on this adapter; a later live run loaded the project's GGUF and completed a model turn with that fallback. Treat acceleration as unverified until a browser/backend exposes concrete offload evidence or a clear speed gain.
