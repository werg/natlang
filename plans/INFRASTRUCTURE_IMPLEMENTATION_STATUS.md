...
| TS host | Native TypeScript interpreter, crisp eval with direct native sharing, model/capability callbacks, live Fold streams, desktop bindings, and package | `ts-host/src/native/`, `ts-host/src/browser/`; `ts-host/test/host.test.mjs` |
...
The counts and server-occupancy observations below describe the original
infrastructure implementation run, not the current checkout or a live server.
For current public APIs see [the TS host](../ts-host/README.md),
[frontend applications](../ts-host/FRONTEND_APPLICATIONS.md), and
