...
`npm run test:conformance` drives all 20 program files with reference scripts and the six harness fixtures. The package unit suite includes paired Python checks for finite crisp outcomes and typed writes, plus native host, cancellation, effect, and snapshot tests. These checks do **not** establish full differential parity. The native model tool schema and prompt are still approximations of `ToolSurface`; diagnostics and trace events are not checked byte-for-byte against Python; semantic review, scenario replay, source/type/run meta-capabilities, and bounded parallel Map are not implemented. The native eval hook is synchronous, so asynchronous declared capability callbacks require a separate async execution design. The browser portability gate is also open. `NatlangHost` remains the Python-backed default export while `NativeNatlangHost` is opt-in.
