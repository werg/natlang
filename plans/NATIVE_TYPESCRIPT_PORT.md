...
The first `ts-host` implementation used a Python interpreter bridge. The native reducer now backs the sole `NatlangHost` implementation, and the bridge has been removed. Paired tests invoke the Python interpreter independently as a reference. The smaller `web/natlang_lite.mjs` remains separate.
...
6. Run the supported structured Python conformance corpus as paired differential fixtures. Make the native runtime the sole TypeScript host implementation.
