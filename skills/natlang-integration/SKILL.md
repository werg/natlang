...
2. Pass typed values or checked source definitions to the runtime. Bind large sparse read-only namespaces as host-backed implementations of ordinary `Dict<T>` inputs; keep this laziness invisible to natlang source. Expose active host operations through crisp helpers or declared effect callbacks. Let natlang call them repeatedly and inspect observations. Host objects may hold databases, jobs, binary data, or a stronger model client without extending the language core.
