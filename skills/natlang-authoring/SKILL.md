...
- Imported functions can be inspected and edited through function tools; the fixed source set cannot be created, deleted, moved, or renamed. Directory reducers alone receive file tools and a writable copy of their input folder. Reducer paths are relative with no prefix. They may create, edit, move, and delete files, and retain changes through their result, `commit`, or `folder.apply`.
