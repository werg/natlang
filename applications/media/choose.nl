---
args:
  request: MediaRequest
  source: Clip
  files?: Folder
returns: Plan
---
Choose exactly one of trim, crop, scale, or transcode from the user's request.
If the request names a sidecar note, read only that file from files before
choosing. Copy request.input and request.output exactly. For trim, use
start/end seconds in source duration. For crop, use x/y and desired width and
height within the source frame. For scale, set positive target width/height.
Use zero for unused numeric fields. Keep audio when the request does not ask to
remove it and the source has audio. Do not claim to understand an unsupported
operation by silently selecting a different one; write kind "unsupported".
